import React, { use } from "react";

const Card = ({ ticketData }) => {
  const initialData = use(ticketData);
  console.log(initialData);

  return (
    <div className="grid  md:grid-cols-12 max-w-[1200px] mx-auto mt-10">
      <div className="bg-gray-200 border-2  col-span-9 p-3">
        <h2 className="text-2xl font-bold text-gray-500">Customer Tickets</h2>
        <div className="grid grid-cols-2 gap-5 mt-5 ">
          {initialData.map((data) => {
            console.log(data);

            return (
              <div className="bg-white shadow-md rounded-2xl cursor-pointer p-5 ">
                <div className="flex justify-between items-center gap-3">
                  <h2 className="text-[#001931] text-lg font-bold">
                    {data.title}
                  </h2>
                  <button
                    className={`px-3 rounded-2xl flex justify-between items-center gap-2 
                    ${
                      data.status == "Open"
                        ? "bg-[#b9f8cf]"
                        : data.status == "In-Progress"
                        ? "bg-[#f8f3b9]"
                        : ""
                    }
                    
                    `}
                  >
                    <div
                      className={`w-4 h-4 rounded-full ${
                        data.status == "Open"
                          ? "bg-[#02a53b]"
                          : data.status == "In-Progress"
                          ? "bg-[#febb0c]"
                          : ""
                      }`}
                    ></div>
                    {data.status}
                  </button>
                </div>
                <p className="my-2 text-[#627382]">{data.description}</p>

                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <p className="text-[#627382]">{data.id}</p>
                    <p
                      className={`text-md ${
                        data.priority == "HIGH PRIORITY"
                          ? "text-[#f83044]"
                          : data.priority == "MEDIUM PRIORITY"
                          ? "text-[#febb0c]"
                          : data.priority == "LOW PRIORITY"
                          ? "text-[#02a53b]"
                          : "text-gray-500"
                      }`}
                    >
                      {data.priority}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <p className="text-[#627382]">{data.customer}</p>
                    <p className="text-[#627382]">{data.createdAt}</p>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      </div>
      <div className="bg-purple-200 border-2 h-[400px] col-span-3">
        <div>
          <h2 className="text-2xl font-bold text-gray-500">Task Status</h2>
        </div>
        <div className="mt-30">
          <h2 className="text-2xl font-bold text-gray-500">Resolved Task</h2>
        </div>
      </div>
    </div>
  );
};

export default Card;
